...
<body>
	<aside>
		...
		<ul>
			...
			<li class="actives"><a href="Score.php" class="menu">Score</a></li>
		</ul>
	</aside>
		...
		<br><br><br><br><br><br><br>
			...
			<?php
				...
				if (isset($_POST["id_jadwal"])) {
					$stmtm = $conn->prepare("SELECT mahasiswa.NRP, mahasiswa.NAME, mahasiswa.STUDY_PROGRAM, mahasiswa.GENDER, detail_jadwal.SCORE, jadwal.PERIODE, detail_jadwal.ID_DETAIL FROM mahasiswa, detail_jadwal, jadwal where mahasiswa.NRP = detail_jadwal.NRP and detail_jadwal.ID_JADWAL = :id_jadwal and detail_jadwal.ID_JADWAL = jadwal.ID_JADWAL ORDER BY jadwal.ID_JADWAL");
					$stmtm->bindParam(":id_jadwal",$_POST['id_jadwal']);
				} 
				else{
					$stmtm = $conn->prepare("SELECT mahasiswa.NRP, mahasiswa.NAME, mahasiswa.STUDY_PROGRAM, mahasiswa.GENDER, detail_jadwal.SCORE, jadwal.PERIODE, detail_jadwal.ID_DETAIL FROM mahasiswa, detail_jadwal, jadwal where mahasiswa.NRP = detail_jadwal.NRP and detail_jadwal.ID_JADWAL = jadwal.ID_JADWAL ORDER BY jadwal.ID_JADWAL");
				}
			  	$stmtm->execute();
			    ...
			    foreach ($hasilm as $recordm) {
					...
					<td align=\"center\"><a href=\"FormEditScore.php?NRP=$recordm[NRP]\" class=\"btn btn-info\">Edit</a>&nbsp&nbsp&nbsp<a href=\"Action/delete.php?ID_DETAIL=$recordm[ID_DETAIL]\" class=\"btn btn-danger\">Delete</a></td>
				</tr>";
			    }
			    echo "</table>";
